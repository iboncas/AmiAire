...
DEFAULT_OUTPUT_DIR = "iteration1/output/analysis/gmm"
...
def require_gmm() -> Any:
    try:
        ...
        raise SystemExit(
            "scikit-learn is required for phase 8 GMM benchmarking. "
...
def fit_predict_gmm(
    ...
    probabilities = model.predict_proba(shuffled_features)
    ...
    restored_confidence[order] = np.max(probabilities, axis=1)

    extras = {
        "bic": float(model.bic(shuffled_features)),
        "aic": float(model.aic(shuffled_features)),
    }
    return restored_labels, restored_confidence, extras


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Gaussian Mixture benchmark.")
    ...
    parser.add_argument(
        ...
        default="full,diag",
        help="Comma-separated covariance types for GMM",
    )
    parser.add_argument("--gmm-n-init", type=int, default=10, help="Number of random initializations per fit")
    ...
    covariance_types = parse_csv_list(args.gmm_covariance_types)
    if not covariance_types:
        raise RuntimeError("At least one GMM covariance type must be provided")
    ...
    for feature_space in feature_spaces:
        ...
        for k_value in k_values:
            for covariance_type in covariance_types:
                ...
                labels, confidence, extras = fit_predict_gmm(features, candidate, random_seed=args.random_seed)
                ...
                assignment_rows = build_assignments_rows(
                    ...
                    feature_space=feature_space,
                    method="gmm",
                    method_variant=covariance_type,
                )
    ...
    silhouette_plot = plot_metric_by_k(
        ...
        title="GMM silhouette benchmark",
        label_builder=lambda result: f"{result['feature_space']}::{result['covariance_type']}",
    )
    ...
    summary = {
        ...
        "feature_spaces": feature_spaces,
        "k_values": k_values,
        "gmm_covariance_types": covariance_types,
